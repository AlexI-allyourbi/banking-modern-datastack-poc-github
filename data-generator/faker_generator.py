...
--------------------
This script simulates a live banking environment by continuously generating
synthetic data for Customers, Accounts, and Transactions and inserting it
into the PostgreSQL OLTP database.

Steps:
1. Load environment variables and configuration settings.
2. Connect to the PostgreSQL database.
3. Define a data generation function `run_iteration` that:
    a. Creates synthetic customers.
    b. Opens accounts for these customers.
    c. Generates transactions (deposits, withdrawals, transfers) for the accounts.
4. Enter a main loop to continuously execute data generation iterations (simulating real-time activity).
"""
import time
import psycopg2
...
# Helper to generate a random Decimal amount with 2 decimal places (e.g., 10.50)
...
# -----------------------------
# Step 3: Define Generation Logic
# -----------------------------
def run_iteration():
    """
    Executes a single batch of data generation:
    1. Creates new Customers.
    2. Opens Accounts for those new customers.
    3. Generates Transactions for the new accounts.
    """
    customers = []
    # 1. Generate customers using Faker
    for _ in range(NUM_CUSTOMERS):
        first_name = fake.first_name()
        last_name = fake.last_name()
        email = fake.unique.email()

        # Insert into DB and capture the generated ID
        cur.execute(
    ...
    # 2. Generate accounts for the newly created customers
    accounts = []
    ...
    # 3. Generate transactions (Deposit, Withdrawal, Transfer)
    txn_types = ["DEPOSIT", "WITHDRAWAL", "TRANSFER"]
    for _ in range(NUM_TRANSACTIONS):
        # Pick a random account from the current batch
        account_id = random.choice(accounts)
        txn_type = random.choice(txn_types)
        amount = round(random.uniform(1, MAX_TXN_AMOUNT), 2)
        related_account = None

        # If transfer, pick a different account as the recipient
...
try:
    iteration = 0
    # Infinite loop to simulate continuous data stream (unless --once is used)
    while True:
        iteration += 1
        ...
        if not LOOP:
            print("Running in 'once' mode. Exiting.")
            break
